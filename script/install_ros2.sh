...
# One-command provisioner for Odin CUDA driver on JetPack 6 / Ubuntu 22.04 / ROS 2 Humble.
set -Eeuo pipefail
umask 022
...
if [[ "${VERSION_ID:-}" != "22.04" ]]; then
    echo "错误：ROS 2 Humble 一键脚本要求 Ubuntu 22.04，当前为 ${PRETTY_NAME:-unknown}。" >&2
    echo "Ubuntu 20.04 请使用 install_ros1.sh。" >&2
    exit 1
fi
...
update_rosdep_cache() {
    ...
    if ! run_as_target env \
        ...
        bash -c 'source /opt/ros/humble/setup.bash && rosdep update'; then
...
step 1 "检查 Jetson 与 CUDA"
if [[ ! -f /etc/nv_tegra_release ]]; then
    echo "错误：未检测到 Jetson Linux。" >&2
    exit 1
fi
head -n 1 /etc/nv_tegra_release
if ! head -n 1 /etc/nv_tegra_release | grep -q '# R36'; then
    echo "错误：Ubuntu 22.04 脚本要求 Jetson Linux R36 / JetPack 6。" >&2
    exit 1
fi
...
apt-get update
apt-get install -y ros-humble-ros-base ros-dev-tools python3-rosdep python3-colcon-common-extensions
...
step 7 "编译 ROS 2 CUDA 驱动"
run_as_target bash -c \
    'set -e; export PATH=/usr/local/cuda/bin:$PATH; cd "$1"; ./script/build_ros2.sh' \
    bash "$PACKAGE_DIR"

step 8 "运行 CUDA 自检"
run_as_target bash -c \
    'set -e; source /opt/ros/humble/setup.bash; source "$1/install/setup.bash"; ros2 run odin_ros_driver odin_cuda_smoke_test' \
    bash "$ODIN_WORKSPACE"

echo
echo "ROS 2 Odin CUDA 驱动安装完成。"
